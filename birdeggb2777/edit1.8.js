...
function setControl() {
  ...
  if (chooseColor == 'laser') {
    ...
    this.getByID(`objValue${1}Container`).classList.remove('d-flex');
    this.getByID(`objValue${4}Container`).classList.remove('d-flex');
    this.getByID(`objValue${2}Container`).classList.remove('d-flex');
    this.getByID(`objValue${1}Container`).classList.add('d-flex');
    this.getByID(`objValue${4}Container`).classList.add('d-flex');
    this.getByID(`objValue${2}Container`).classList.add('d-flex');
    getByID('objValue4').value = '150';
    getByID('objValue1').value = '0.5';
    getByID('objValue2').value = '200';
  }
  else {
    for (let index = 1; index < 4 + 1; index++) {
      this.getByID(`objValue${index}Container`).classList.remove('d-flex')
...
function blockEdit() {
  ...
  if (AllObject[NowObj] && AllObject[NowObj].Image && AllObject[NowObj].Image == shootImg) {
    AllObject[NowObj].objValue3 = parseFloat(getByID('objValue4').value);
    AllObject[NowObj].speed = parseFloat(getByID('objValue1').value);
    AllObject[NowObj].objValue = parseFloat(getByID('objValue2').value);
    // alert("");
  }
}
///////////////////
...
shootImg.src = 'shootblock.png';
shootImg.alt = 'shootImg';
...
function chooseLaser() {
  if (tempImg == laserBlockImg) {
    ...
    this.getByID(`objValue${1}Container`).classList.remove('d-none');
...
function chooseLaser() {
  if (tempImg == shootImg) {
    ...
    this.getByID(`objValue${1}Container`).classList.remove('d-none');
    this.getByID(`objValue${4}Container`).classList.remove('d-none');
    this.getByID(`objValue${2}Container`).classList.remove('d-none');
    this.getByID(`objValue${1}Container`).classList.add('d-flex');
    this.getByID(`objValue${4}Container`).classList.add('d-flex');
    this.getByID(`objValue${2}Container`).classList.add('d-flex');
  } else {
...
function MouseMove(e) {
  ...
  if (chooseColor == 'yellow') {
    ...
    tempDrawing = function () {
      oCtx.drawImage(shootImg, chooseX - w / 2, chooseY - h / 2, w, h);
    }
  }
  else {
    check = 1;
...
function MouseUp(e) {
  ...
  else if (chooseColor == 'shoot') tempColor = shootImg;
  else check = 1;
  chooseY = AutoEditPointY(chooseX, chooseY);
  /* if (downCheck == true) {
       for (let i in AllObject) {
  ...
  for (var l in list) {
    let obj = {
      ...
      code: function () {
        ...
        this.pointX += this.speedX;
        this.pointY += this.speedY;

        if (pounch(0, 0, oCanvas.width, oCanvas.height,
          this.pointX, this.pointY, this.width, this.height) == false) {
          AllObject.splice(AllObject.indexOf(this), 1);
        }
      },
      registerDraw: function () {
        oCtx.beginPath();
